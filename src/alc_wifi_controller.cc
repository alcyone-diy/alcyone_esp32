...
esp_err_t WifiController::Init() {
  ...
  // 3. Configuration du driver Wi-Fi avec les réglages par défaut
  ...
  last_error_ = esp_event_handler_instance_register(WIFI_EVENT,
                                                    ESP_EVENT_ANY_ID,
                                                    &WifiController::EventHandler,
                                                    this,
                                                    &wifi_event_handler_instance_);
  if (last_error_ != ESP_OK) {
    ESP_LOGE(TAG, "esp_event_handler_instance_register(WIFI_EVENT) error: %s", esp_err_to_name(last_error_));
    return last_error_;
  }
  ...
  if (last_error_ != ESP_OK) {
    ESP_LOGE(TAG, "esp_event_handler_instance_register(IP_EVENT) error: %s", esp_err_to_name(last_error_));
    return last_error_;
  }
...
esp_err_t WifiController::SetDefaultWifi() {
  if (credential_list_.size() == 0) {
    return ESP_ERR_INVALID_ARG;
  }
  if (credential_list_.size() <= credential_index_) {
    credential_index_ = 0;
  }
  Credential cerdential = credential_list_[credential_index_];

  ESP_LOGI(TAG, "Connect: %s", cerdential.ssid.c_str());
  wifi_config_t wifi_config = {}; // Initialise tout à zéro, important !
  
  // Utilise strlcpy au lieu de strncpy
  strlcpy(reinterpret_cast<char*>(wifi_config.sta.ssid), cerdential.ssid.c_str(), sizeof(wifi_config.sta.ssid));
  strlcpy(reinterpret_cast<char*>(wifi_config.sta.password), cerdential.password.c_str(), sizeof(wifi_config.sta.password));
...
void WifiController::UpdateState(WifiController::State state, WifiController::LastConnection last_connection) {
...
void WifiController::OnStaDisconnectEvent(wifi_event_sta_disconnected_t *event_data) {
  // 4WAY_HANDSHAKE_TIMEOUT, HANDSHAKE_TIMEOUT, CONNECTION_FAIL mauvais mot de passe
  // NO_AP_FOUND wifi not found
  // NO_AP_FOUND_AUTHMODE auth problem
  wifi_err_reason_t reason = static_cast<wifi_err_reason_t>(event_data->reason);
  ESP_LOGW(TAG, "Déconnexion. Raison : %d", reason);
  
  State new_state = (state_ == State::kGotIp || state_ == State::kReconnecting)
                        ? State::kReconnecting : State::kIdle;
  ...
  if (reason == WIFI_REASON_AUTH_FAIL) {
    ...
    ESP_LOGE(TAG, "Mot de passe invalide. Abandon.");
  ...
  if (new_state == State::kReconnecting) {
    retry_num_++;
    ESP_LOGW(TAG, "Reconnect %s", retry_num_);
    // On définit un délai de grâce pour laisser le hardware souffler
    uint32_t retry_delay_us = (retry_num_ <= 2) ? 2'000'000 : 5'000'000;
    
    // Optionnel mais propre : on peut faire un esp_wifi_stop/start 
    // si la raison est vraiment bizarre (codes > 100)
    if (reason > 100) {
       ESP_LOGD(TAG, "Erreur interne détectée, reset radio léger.");
    }
...
void WifiController::OnScanDoneEvent(wifi_event_sta_scan_done_t* event_data) {
  ...
  // Utilisation d'un vector pour la sécurité mémoire
...
void WifiController::HandleRetry() {
  ESP_LOGI(TAG, "Le timer a expiré, tentative de reconnexion…");
  esp_wifi_connect();
}
  ...
  if (event_base == WIFI_EVENT) {
    ...
    switch(wifi_event) {
      ...
      case WIFI_EVENT_STA_DISCONNECTED:
        ...
        wifi_controller->OnStaDisconnectEvent(static_cast<wifi_event_sta_disconnected_t*>(event_data));
        break;
