...
esp_err_t BNO086::ReceivePacket(uint16_t timeout_ms) {
    ...
    // Read the 4-byte SHTP header to determine length
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
...
void BNO086::ParsePacket() {
    ...
    if (channel == CHANNEL_REPORTS || channel == CHANNEL_WAKE_REPORTS) {
        ...
        // Here we could handle command responses, but for basic use we skip
    }
}
...
void BNO086::ParseSH2Report(uint8_t* payload, uint16_t len) {
    ...
    while (curr < len) {
        ...
        switch (report_id) {
            ...
            default:
                // Unknown report or padding, skip to end
                curr = len;
                break;
        }
    }
...
esp_err_t BNO086::EnableAccelerometer(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_ACCELEROMETER, period_us); }
esp_err_t BNO086::EnableGyroscope(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_GYROSCOPE, period_us); }
esp_err_t BNO086::EnableMagnetometer(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_MAGNETIC_FIELD, period_us); }
esp_err_t BNO086::EnableLinearAcceleration(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_LINEAR_ACCELERATION, period_us); }
esp_err_t BNO086::EnableGravity(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_GRAVITY, period_us); }
esp_err_t BNO086::EnableRotationVector(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_ROTATION_VECTOR, period_us); }
esp_err_t BNO086::EnableGameRotationVector(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_GAME_ROTATION_VECTOR, period_us); }
esp_err_t BNO086::EnableARVRStabilizedRotationVector(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_ARVR_STABILIZED_ROTATION_VECTOR, period_us); }
esp_err_t BNO086::EnableARVRStabilizedGameRotationVector(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_ARVR_STABILIZED_GAME_ROTATION_VECTOR, period_us); }
esp_err_t BNO086::EnableStepCounter(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_STEP_COUNTER, period_us); }
esp_err_t BNO086::EnableStabilityClassifier(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_STABILITY_CLASSIFIER, period_us); }
esp_err_t BNO086::EnableGyroIntegratedRotationVector(uint32_t period_us) { return SetFeature(SENSOR_REPORTID_GYRO_INTEGRATED_ROTATION_VECTOR, period_us); }
