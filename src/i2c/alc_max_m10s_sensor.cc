...
namespace {
constexpr char kTag[] = "ALC_MaxM10sSensor";
}
...
void MaxM10sSensor::Init(Callback cb) {
  bus_manager_.Enqueue([this](BusToken& token) -> esp_err_t {
    ESP_LOGI(kTag, "Initializing MaxM10sSensor at address 0x%02X", address_);
    ...
    if (err != ESP_OK) {
      ESP_LOGE(kTag, "Failed to connect to MaxM10sSensor: %s", esp_err_to_name(err));
      return err;
    }
