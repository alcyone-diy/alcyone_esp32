...
void ESP32Timer::Stop() {
  std::lock_guard<std::recursive_mutex> lock(state_->mutex);
  if (!state_->handle || !state_->is_running) return;
  esp_err_t err = esp_timer_stop(state_->handle);
  if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
    ESP_LOGW("TIMER", "Erreur inattendue lors du Stop: %s", esp_err_to_name(err));
  }
...
void ESP32Timer::StaticCallback(void* arg) {
  ...
  if (!locked_state) return;
  {
    ...
    if (!locked_state->is_running) return;
    if (!locked_state->is_periodic) locked_state->is_running = false;
  }
